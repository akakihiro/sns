...
FriendsForm,CreateGroupForm,PostForm,OrderForm,EntryForm
...
def index(request,page=1):
    ...
    if request.method=='POST':
        ...
        for item in request.POST.getlist('groups'):
            glist.append(item)
        #Messageの取得
        messages=get_your_group_message(request,glist,page)
...
def add(request):
    ...
    if add_user==request.user:
        messages.info(request,"自分をFriendに追加することは\
                できません。")
...
def entry(request):
    if request.method == 'POST':
        ...
        if entryform.is_valid():
            entryform.save()
            return redirect(to='/admin/logout?next=/sns')
    else:
        entryform = EntryForm()

    params = {'entryform':entryform}
    return render(request,'sns/entry.html',params)


#これ以降は普通の関数====================

#指定されたグループおよび検索文字によるMessageの取得
def get_your_group_message(request,glist,page):
    page_num=10 #ページあたりの表示数
    #publicの取得
    (public_user,public_group)=get_public()
    #チェックされたGroup取得
    groups=Group.objects.filter(Q(owner=request.user)\
    ...
    his_friends=Friend.objects.filter(user=request.user)\
    ...
    if request.method=='POST':
        if request.POST['order']=='ASC':
            messages=Message.objects.filter(Q(group__in=groups)\
                |Q(group__in=me_groups)).order_by('pub_date')
        else:
            messages=Message.objects.filter(Q(group__in=groups)\
